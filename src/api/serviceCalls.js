...
export const signInWithGoogle = async (studentDetails) => {
  try {
    console.log("logging email");
    console.log(studentDetails.email);
    const response = await service.post("/users/signInWithGoogle", {
      email: studentDetails.email,
    });
    return response;
  } catch (err) {
    console.log("error in SignInWithGoogle function: " + err);
    return err;
  }
};
...
export const sendTeacherRequest = async (
  studentId,
  coursesIds,
  price,
  privateInfo
